...
本文使用\texttt{Energy Delay Product(EDP)}评估CNN前向推断过程所涉及操作的执行能效，\ref{chapter:edp}节对其概念及选择原因进行了阐述。由于本文其他章节的实验部分都是基于ODROID-XU3\cite{hardkernel.com}和Open-Q™ 820开发套件\cite{intrinsyc.com}两个平台进行的，故而对\ref{chapter:chapter2-5-1}节和\ref{chapter:chapter2-5-2}节分别对两个平台进行了详细介绍。
...
本文中，\texttt{E}代表平均能耗，\texttt{T}代表平均执行时间。很明显，\texttt{EDP}是一个值越小越好的度量标准。
